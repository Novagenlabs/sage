  ...
  return (
    <div className="v2-screen bg-chamber-900">
      <div className="flex justify-between items-start mb-6">
        ...
        <Link
          href="/home"
          className="h-9 w-9 rounded-full bg-chamber-800 flex items-center justify-center text-chamber-300"
          aria-label="close"
        >
          <X className="h-5 w-5" />
        </Link>
      ...
      <p className="v2-sub mb-6 max-w-sm">
        credits power voice and chat. one credit ≈ one second of voice or a
        short text exchange. video is ~3× voice.
      </p>

      {error && (
        <div className="rounded-2xl bg-red-500/10 border border-red-500/30 p-3 mb-4 text-sm text-red-300 lowercase text-center">
          {error}
        </div>
      )}
        ...
        {CREDIT_PACKAGES.map((pkg, i) => {
          ...
          return (
            <button
              ...
              onClick={() => buy(pkg)}
              disabled={anyLoading}
              className={`relative w-full text-left rounded-3xl p-5 transition-all disabled:opacity-50 ${
                featured
      ...
      <Link
        href="/credits"
        className="block text-center text-sm text-chamber-400 mt-6 lowercase"
      >
