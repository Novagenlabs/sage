...
function EntriesInner() {
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      router.replace("/v2/auth/signin?next=/v2/entries");
    }
  }, [status, router]);
  ...
  return (
    <>
      {/* ====== MOBILE — full-screen list, navigate to /v2/entries/[id] ====== */}
              ...
              renderItem={(c, i) => {
                ...
                return (
                  <button
                    key={c.id}
                    onClick={() =>
                      router.replace(`/v2/entries?id=${c.id}`, {
                        scroll: false,
                      })
                    }
            ...
            {selectedId ? (
              <EntryDetailPane
                ...
                onDeleted={() => {
                  ...
                  router.replace("/v2/entries", { scroll: false });
                }}
              />
            ) : (
              <EmptyDetail />
