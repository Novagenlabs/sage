  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      const next = encodeURIComponent(
        seedPrompt
          ? `/v2/chat/text?prompt=${encodeURIComponent(seedPrompt)}${seedTitle ? `&title=${encodeURIComponent(seedTitle)}` : ""}`
          : "/v2/chat/text"
      );
      router.replace(`/v2/auth/signin?next=${next}`);
    }
  ...
  useEffect(() => {
    if (
      ...
      (async () => {
        ...
        router.replace(`/v2/chat/text${qs ? `?${qs}` : ""}`, { scroll: false });
        setFreshDone(true);
      })();
    }
  }, [
